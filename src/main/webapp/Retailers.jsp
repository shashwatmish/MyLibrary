...
<body>
	...
	<br><br>
		...
		<c:if test="${manager ne null && Dept eq 102}">
			<a class="btn btn-primary" href="/registerretailer" style="left: 390px;">Add Retailer</a>
		</c:if>
		<c:if test="${manager ne null && Dept eq 102}">
			<a class="btn btn-primary" href="/updateretailer" style="left: 380px;">Update Retailer</a>
		</c:if>
	
	<div class="list-group">
		<h1>
			<label style="color: green; position: relative; left: 45%;">Retailers
			</label>
		</h1>
		<br>
		<form class="form-inline my-2 my-lg-0" action="/getretailer" method="get" style="justify-content: center;">
			<input style="border:1px solid green; width:350px;" class="form-control mr-sm-2 " type="text"
				placeholder="Enter Retailer ID to be searched" aria-label="Search" style="width: 50%;"
				name="id">
		...
		<c:if test="${manager ne null && Dept eq 102}">
			<br>
			<form class="form-inline my-2 my-lg-0" action="/deleteretailer" method="post" style="justify-content: center;">
			<input style="border:1px solid green; width:350px;" class="form-control mr-sm-2 " type="text"
				placeholder="Enter Retailer ID to be removed" aria-label="Delete" style="width: 50%;"
				name="id">
		...
		<c:if test="${ error ne null }">
			<p>THIS RETAILER DOESN'T EXIST</p>
		</c:if>
		...
		<c:if test="${ qq ne null }">
			<div class="card border border-success">
				...
				<div class="card-body">
					<h5 class="card-title">
						Retailer ID: <span style="color: #6c757d !important">${qq.retailerid}</span>
					</h5>
					<h5 class="card-title">
		...
		<c:forEach begin="0" end="${qresult.size()}" var="q" items="${qresult}">
			...
			<div class="card border border-success">
				...
				<div class="card-body">
					<h5 class="card-title">
						Retailer ID: <span style="color: #6c757d !important">${q.retailerid}</span>
					</h5>
					<h5 class="card-title">
