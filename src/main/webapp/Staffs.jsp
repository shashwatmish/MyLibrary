...
<body>
	...
	<br><br>
		...
		<c:if test="${manager ne null && Dept eq 104}">
			<a class="btn btn-primary" href="/updatestaff" style="left: 380px;">Update Staff</a>
		</c:if>
	
	<div class="list-group">
		<h1>
			<label style="color: green; position: relative; left: 45%;">Staff Members
			</label>
		</h1>
		<form class="form-inline my-2 my-lg-0" action="/getstaff" method="get" style="justify-content: center;">
			<input style="border:1px solid green; width:350px;" class="form-control mr-sm-2 " type="text"
				placeholder="Enter Staff ID to be searched" aria-label="Search" style="width: 50%;"
				name="id">
		...
		<c:if test="${manager ne null && Dept eq 104}">
			<br>
			<form class="form-inline my-2 my-lg-0" action="/deletestaff" method="post" style="justify-content: center;">
			<input style="border:1px solid green; width:350px;" class="form-control mr-sm-2 " type="text"
				placeholder="Enter Staff ID to be removed" aria-label="Delete" style="width: 50%;"
				name="id">
		...
		<c:if test="${ qq ne null }">
			<div class="card border border-success">
				<div class="card-body">
					<h5 class="card-title">
						Staff ID: <span style="color: #6c757d !important">${qq.staffid}</span>
					</h5>
					<h5 class="card-title">
					...
					<h5 class="card-title">
						No. of shelves handled :<span style="color: #6c757d !important">${qq.handlesshelf}</span>
					</h5>
				</div>
			</div>
		...
		<c:forEach begin="0" end="${qresult.size()}" var="q" items="${qresult}">
			...
			<div class="card border border-success">
				...
				<div class="card-body">
					<h5 class="card-title">
						Staff ID: <span style="color: #6c757d !important">${q.staffid}</span>
					</h5>
					<h5 class="card-title">
					...
					<h5 class="card-title">
						No. of shelves handled :<span style="color: #6c757d !important">${q.handlesshelf}</span>
					</h5>
				</div>
			</div>
		</c:forEach>
